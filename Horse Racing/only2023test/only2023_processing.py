...
'''
def convert_mixed_fraction_to_float(fraction_str):
...
#relative_dfについて、日本語名と英語名が存在するため、英語名の消去を行う。のちのち、ベクトル変化するため、そっちのコードで使うかも
for column in relative_df.columns.tolist():
  relative_df[column] = relative_df[column].apply(remove_english_parentheses)
...
'''
df['gap'] = df['gap'].replace(np.nan,'0')
df['gap'] = df['gap'].str.replace('ハナ','1/12')
df['gap'] = df['gap'].str.replace('アタマ','1/6')
df['gap'] = df['gap'].str.replace('クビ','1/3')
df['gap'] = df['gap'].str.replace('大','11')
df['gap'] = df['gap'].str.replace('同着','0')
df['gap'] = df['gap'].apply(lambda x: 2.4*convert_mixed_fraction_to_float(x))
'''
...
#p_rankには最大4つまでの順位が乗っているので、それぞれ分割
df['first_pass_rank'] = df['p_rank'].str.split('-').str[0]
df['second_pass_rank'] = df['p_rank'].str.split('-').str[1]
df['third_pass_rank'] = df['p_rank'].str.split('-').str[2]
df['fourth_pass_rank'] = df['p_rank'].str.split('-').str[3]

#one-hot化したもの、綺麗にしたもの、追加したものなどがあるため、必要なだけにした。
horse_df = df[['year','race_id','length','out_length','tan','fuku_1','fuku_2','fuku_3','uren','wide_1_2','wide_1_3','wide_2_3','utan','sanfuku','santan','rank','name','jockey','age','e_weight','gap','first_pass_rank','second_pass_rank','third_pass_rank','fourth_pass_rank','odds','pop','h_weight','weight_change']]
